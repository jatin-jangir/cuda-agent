...
static void redirect_pte_function(struct vm_area_struct *vma, unsigned long addr, unsigned int flags, struct pt_regs *regs) {
    ...
    if (addr < dst_start || addr >= dst_end)
    {
	pr_info("addr 0x%lx is not valid for redirect\n",addr);    
	    return;
    }
    ...
    pr_info("vm_page_prot: 0x%lx\n", pgprot_val(vma->vm_page_prot));
    pr_info("vm_ops: %pS\n", vma->vm_ops);
    pr_info("anon_vma: %px\n", vma->anon_vma);
    pr_info("vm_file: %px\n", vma->vm_file);
    pr_info("vm_private_data: %px\n", vma->vm_private_data);

    pr_info("=== Page Table Walk for addr: 0x%lx ===\n", addr);
    ...
    pr_info("=== Page Struct Info ===\n");
    pr_info("Page PFN: 0x%lx\n", pfn);
    ...
    pr_info("page_owner info (if enabled)\n");
#endif

    pte_unmap_unlock(pte, ptl);

    pr_info("=== Additional Context ===\n");
    pr_info("Fault flags: 0x%x\n", flags);
    pr_info("Faulting instruction pointer: 0x%lx\n", instruction_pointer(regs));
}
*/
