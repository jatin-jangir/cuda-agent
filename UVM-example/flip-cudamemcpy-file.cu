...
int main() {
    ...
    // 3. Create UVM array that shares the same physical memory
    char* uvm_data;
    cudaHostRegister(file_data, sb.st_size, cudaHostRegisterMapped);
    cudaHostGetDevicePointer(&uvm_data, file_data, 0);
    ...
    for (int i = 0; i < rows; i++) {
        row_offsets[i] = ptr - file_data;
        while (*ptr++ != '\n');
    }
    
    // Copy row offsets to device
    cudaMalloc(&d_row_offsets, rows * sizeof(int));
    cudaMemcpy(d_row_offsets, row_offsets, rows * sizeof(int), cudaMemcpyHostToDevice);
    ...
    for (int row = 0; row < rows; row++) {
        ...
        // Explicitly flush changes from GPU to CPU view
        cudaStream_t stream;
        cudaStreamCreate(&stream);
        cudaMemPrefetchAsync(file_data, sb.st_size, cudaCpuDeviceId, stream);
        cudaStreamSynchronize(stream);
        cudaStreamDestroy(stream);
	printf("going to sleep after updating %d row \n",row);
	sleep(20);
    }

    // 6. Sync changes back to the file
